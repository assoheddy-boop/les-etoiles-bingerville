...
import { classGenderCounts, enrollmentForStudent } from "@/lib/enrollment";
import { buildEnrollmentFichePdf } from "@/lib/enrollment-pdf";
...
export async function GET(request: Request, context: { params: Promise<{ studentId: string }> }) {
  ...
  const result = await buildEnrollmentFichePdf({
    student,
    enrollment: enrollmentForStudent(studentId, data),
    data,
    effectif: classGenderCounts(student.classId, data),
  });
